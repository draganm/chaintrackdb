...
func (f *fragmentAggregator) toSegment() (store.Address, error) {
	...
	sw, err := f.store.AppendBlock(store.TypeDataNode, len(f.fragments), 8)
	if err != nil {
...
func (f *fragmentAggregator) finish() (store.Address, error) {
	if f.parent == nil {
		if len(f.fragments) == 0 {
			sw, err := f.store.AppendBlock(store.TypeDataLeaf, 0, 0)
...
func (dw *DataWriter) Write(d []byte) (int, error) {
	...
	for len(d) > 0 {
		...
		if len(dw.buffer) == dw.fragSize {
			sw, err := dw.store.AppendBlock(store.TypeDataLeaf, 0, len(dw.buffer))
			if err != nil {
